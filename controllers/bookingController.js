...
const createBookingCheckout = async (session) => {
	...
	const price = session.display_items[0].amount / 100; // display_items is line_items.
	await Booking.create({ tour, user, price });
};
...
exports.webhookCheckout = (req, res, next) => {
	...
	if (event.type === 'checkout.session.completed') createBookingCheckout(event.dat.object);
